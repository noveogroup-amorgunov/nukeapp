...
const injectedRtkApi = api.injectEndpoints({
  endpoints: (build) => ({
    ...
    login: build.mutation<LoginApiResponse, LoginApiArg>({
      ...
      invalidatesTags: ["session", "wishlist"],
    }),
    ...
    >({
      query: (queryArg) => ({
        url: `/feature-toggle`,
        params: {
          canTurnDarkMode: queryArg.canTurnDarkMode,
          canSortProducts: queryArg.canSortProducts,
        },
      }),
    }),
...
export type UpdateCartApiArg = {
  /** Mock-only response delay in ms, ignored by a real backend */
  delay?: number;
...
export type GetFeatureToggleApiArg = {
  canTurnDarkMode?: BooleanAsString;
  canSortProducts?: BooleanAsString;
};
